...
use crate::structural_mirror::types::{
    InfluenceScore, KnowledgeCluster, KnowledgeGap, StructureMap,
};
...
// ─────────────────────────────────────────────────────────────────────────────
// Full StructureMap construction
// ─────────────────────────────────────────────────────────────────────────────

pub fn build_structure_map(blocks: &[Block]) -> StructureMap {
...
mod tests {
    ...
    fn test_density_all_connected() {
        let page_id = uuid_from_u8(1);
        let ids: Vec<Uuid> = (0..5).map(|i| uuid_from_u8(i)).collect();
    ...
    #[test]
    fn test_build_structure_map_empty() {
        let map = build_structure_map(&[]);
    ...
    #[test]
    fn test_build_structure_map_single_block() {
        let page_id = uuid_from_u8(1);
        let block = make_block(uuid_from_u8(10), vec![], page_id);
        let map = build_structure_map(&[block]);
