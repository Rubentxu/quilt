...
/// The Structure Mapper analyzes a page's blocks to build structure graphs
...
impl std::fmt::Debug for StructureMapper {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("StructureMapper")
...
impl StructureMapper {
    ...
    ///
    /// Builds a complete structure graph for the given page by:
    /// 1. Fetching all blocks on the page
    /// 2. Classifying each block as Claim/Evidence/Rebuttal/etc via heuristic analysis
    ...
    pub async fn map_arguments(
        &self,
        page_id: Uuid,
    ) -> Result<StructureGraph, StructureMapperError> {
        let blocks = self
            .block_repo
            .get_by_page(page_id)
            .await
        ...
        // Phase 2: Build structure edges from block refs
    ...
    /// Detect arguments in a set of blocks via heuristic analysis.
    ...
    /// Classify a single block's argument role via heuristic analysis.
    async fn classify_block(&self, block: &Block) -> ArgumentDetection {
        // Simple heuristic approach
        let content = &block.content;

        // Quick heuristic pre-filter
        let (classification, confidence) = self.heuristic_classify_block(content).await;
    ...
    /// Classify a block's argument role using heuristics (per ADR-0001: no AI/LLM).
    async fn heuristic_classify_block(&self, content: &str) -> (ArgumentRole, f64) {
        let classification = classify_heuristic(content);
        let confidence = 0.6; // Base confidence for heuristic
        (classification, confidence)
    }

    /// Build structure nodes from blocks and detections.
    fn build_nodes(
        &self,
        blocks: &[Block],
        detections: &[ArgumentDetection],
    ) -> Vec<StructureNode> {
    ...
    /// Build structure edges from block references.
    fn build_edges(&self, blocks: &[Block], nodes: &[StructureNode]) -> Vec<StructureEdge> {
    ...
    pub async fn detect_contradictions(
        &self,
        page_id: Uuid,
    ) -> Result<Vec<ContradictionPair>, StructureMapperError> {
        ...
        for (i, a) in claims.iter().enumerate() {
            for b in claims.iter().skip(i + 1) {
                // Use heuristic to determine if claims are contradictory
    ...
    /// Assess whether two blocks are in conflict using heuristic analysis.
    ...
    /// Detect logical fallacies in a block via heuristic analysis.
    #[instrument(skip(self))]
    pub async fn detect_fallacies(
        &self,
        block_id: Uuid,
    ) -> Result<Vec<DetectedFallacy>, StructureMapperError> {
        let block = self
            .block_repo
            .get_by_id(block_id)
            .await
            .map_err(AnalysisError::Storage)?
            .ok_or(AnalysisError::BlockNotFound(block_id.to_string()))?;

        // Try heuristic analysis
        let fallacies = self.heuristic_detect_fallacies(&block).await;

        Ok(fallacies)
    }

    /// Heuristic-driven fallacy detection for a block.
    async fn heuristic_detect_fallacies(&self, block: &Block) -> Vec<DetectedFallacy> {
        // Use heuristics to detect fallacies in block content
    ...
    pub async fn score_argument_strength(
        &self,
        block_id: Uuid,
    ) -> Result<f64, StructureMapperError> {
        let block = self
            .block_repo
            .get_by_id(block_id)
            .await
        ...
        // Coherence score from heuristic
...
/// Errors for the Structure Mapper.
pub type StructureMapperError = AnalysisError;
...
/// Compute the position (in/out/neutral) of a block in the structure graph.
...
fn calculate_ref_depth(node_id: Uuid, edges: &[StructureEdge], max_depth: usize) -> usize {
    let mut depth = 0;
...
fn detect_cycle(node_id: Uuid, edges: &[StructureEdge]) -> bool {
    let mut visited = HashSet::new();
...
mod tests {
    ...
    struct MockBlockRepo {
        pages: std::collections::HashMap<Uuid, Vec<Block>>,
        blocks: std::collections::HashMap<Uuid, Block>,
    }

    impl MockBlockRepo {
        fn from_blocks(page_id: Uuid, blocks: Vec<Block>) -> Self {
            let blocks_map: std::collections::HashMap<Uuid, Block> =
    ...
    async fn test_map_arguments_empty_page() {
        ...
        let mapper = StructureMapper::new(repo);

        let graph = mapper.map_arguments(page_id).await.unwrap();
    ...
    async fn test_detect_contradictions_no_claims() {
        ...
        let contradictions = mapper.detect_contradictions(page_id).await.unwrap();
    ...
    async fn test_score_argument_strength_unsupported() {
        ...
        let score = mapper.score_argument_strength(block_id).await.unwrap();
        assert!(score < 0.3);
    }
    ...
    async fn test_score_argument_strength_well_supported() {
        ...
        let score = mapper.score_argument_strength(claim).await.unwrap();
