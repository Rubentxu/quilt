...
//!
//! Ported from `quilt-sync/src/crdt.rs` for WASM-native targets.
//!
//! # Conflict Resolution
//!
...
pub enum ConflictError {
    ...
    /// Manual resolution required for entity
    RequiresManualResolution { entity_id: Uuid },
    /// Conflict marker creation failed
...
impl std::fmt::Display for ConflictError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ConflictError::ConcurrentModification { entity_id } => {
                write!(f, "Concurrent modification detected for entity {entity_id}")
            }
            ConflictError::RequiresManualResolution { entity_id } => {
                write!(f, "Manual resolution required for entity {entity_id}")
            }
            ConflictError::MarkerCreationFailed { reason } => {
                write!(f, "Conflict marker creation failed: {reason}")
            }
        }
    }
}
