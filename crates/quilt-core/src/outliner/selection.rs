...
impl SelectionState {
    ...
    /// Clear editing only if the given block_id is currently being edited.
    /// This prevents clearing a new block's editing when a different block's blur fires.
    pub fn clear_editing_if(&self, block_id: &str) {
        if self.editing_block_id.get().as_deref() == Some(block_id) {
            self.editing_block_id.set(None);
        }
    }
