...
///     HandlerContainer, DefaultBlockHandler, DefaultPageHandler,
///     DefaultSearchHandler, DefaultDeepLinkHandler, DefaultSettingsHandler,
/// };
///
/// // Example with repositories and services
/// let handlers = HandlerContainer::new()
///     .with_block_handler(DefaultBlockHandler::new(block_repo, timezone_service))
///     .with_page_handler(DefaultPageHandler::new(block_repo, page_repo, timezone_service))
///     .with_search_handler(DefaultSearchHandler::new(search_service, None, None))
///     .with_deep_link_handler(DefaultDeepLinkHandler::new(deep_link_repo))
///     .with_settings_handler(DefaultSettingsHandler::new(settings_repo));
/// ```
#[derive(Clone)]
pub struct HandlerContainer {
    block: Option<Arc<dyn BlockHandler>>,
    page: Option<Arc<dyn PageHandler>>,
    search: Option<Arc<dyn SearchHandler>>,
    deep_link: Option<Arc<dyn DeepLinkHandler>>,
    settings: Option<Arc<dyn SettingsHandler>>,
    daily_summary: Option<Arc<dyn DailySummaryHandler>>,
    cognitive: Option<Arc<dyn CognitiveHandler>>,
}

impl Default for HandlerContainer {
    fn default() -> Self {
        Self::new()
    }
}
...
impl HandlerContainer {
    /// Create a new empty HandlerContainer.
    pub fn new() -> Self {
        Self {
            block: None,
            page: None,
    ...
    pub fn with_block_handler(mut self, handler: impl BlockHandler + 'static) -> Self {
        self.block = Some(Arc::new(handler));
        self
    }

    /// Set the page handler.
    pub fn with_page_handler(mut self, handler: impl PageHandler + 'static) -> Self {
        self.page = Some(Arc::new(handler));
        self
    }

    /// Set the search handler.
    pub fn with_search_handler(mut self, handler: impl SearchHandler + 'static) -> Self {
        self.search = Some(Arc::new(handler));
        self
    }

    /// Set the deep link handler.
    pub fn with_deep_link_handler(mut self, handler: impl DeepLinkHandler + 'static) -> Self {
        self.deep_link = Some(Arc::new(handler));
        self
    }

    /// Set the settings handler.
    pub fn with_settings_handler(mut self, handler: impl SettingsHandler + 'static) -> Self {
        self.settings = Some(Arc::new(handler));
        self
    }
    ...
    /// Set the cognitive handler.
    pub fn with_cognitive_handler(mut self, handler: impl CognitiveHandler + 'static) -> Self {
        self.cognitive = Some(Arc::new(handler));
        self
    }

    /// Get the block handler.
    pub fn block(&self) -> Option<&Arc<dyn BlockHandler>> {
        self.block.as_ref()
    }

    /// Get the page handler.
    pub fn page(&self) -> Option<&Arc<dyn PageHandler>> {
        self.page.as_ref()
    }

    /// Get the search handler.
    pub fn search(&self) -> Option<&Arc<dyn SearchHandler>> {
        self.search.as_ref()
    }

    /// Get the deep link handler.
    pub fn deep_link(&self) -> Option<&Arc<dyn DeepLinkHandler>> {
        self.deep_link.as_ref()
    }

    /// Get the settings handler.
    pub fn settings(&self) -> Option<&Arc<dyn SettingsHandler>> {
        self.settings.as_ref()
    }

    /// Get the daily summary handler.
    pub fn daily_summary(&self) -> Option<&Arc<dyn DailySummaryHandler>> {
        self.daily_summary.as_ref()
    }

    /// Get the cognitive handler.
    pub fn cognitive(&self) -> Option<&Arc<dyn CognitiveHandler>> {
...
pub trait CognitiveHandler: Send + Sync {
    ...
    /// Get cognitive map statistics across recent pages.
    ...
    /// Get argument structure for a page.
    async fn resource_arguments(&self, uri: &str) -> HandlerResult;

    /// Get mental models from journals.
