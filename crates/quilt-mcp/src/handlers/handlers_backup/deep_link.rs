...
impl DeepLinkHandler for DefaultDeepLinkHandler {
    ...
    async fn get_deep_links(
        ...
        let links = self
            .deep_link_repo
            .list_by_filter(source_id, target_id, link_type.as_deref(), limit)
            .await
            .map_err(|e| e.to_string())?;
        ...
        let items: Vec<serde_json::Value> = links
            ...
            .map(|l| {
                serde_json::json!({
                    "id": l.id.to_string(),
                    "source_id": l.hierarchy.source_id.to_string(),
                    "source_type": format!("{:?}", l.hierarchy.source_type).to_lowercase(),
                    "target_id": l.target.target_id.map(|id| id.to_string()),
                    "target_page_name": l.target.target_page_name,
                    "link_type": format!("{:?}", l.target.link_type).to_lowercase(),
                    "external_url": l.target.external_url,
                    "link_text": l.target.link_text,
                    "context": l.target.context,
                })
            })
            .collect();
