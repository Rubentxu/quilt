...
use super::{HandlerResult, SearchHandler as SearchHandlerTrait, SearchResult};
...
impl SearchHandlerTrait for DefaultSearchHandler {
    ...
    async fn rebuild_index(&self, mode: Option<String>, since: Option<String>) -> HandlerResult {
        ...
        match mode {
            "incremental" => {
                let since = since.ok_or_else(|| "Since timestamp required for incremental mode".to_string())?;
                index
                    .rebuild_incremental(&since)
                    .await
                    .map_err(|e| e.to_string())?;
            }
            "full" | _ => {
    ...
    async fn index_health(&self) -> HandlerResult {
        ...
        Ok(serde_json::to_string_pretty(&serde_json::json!({
            "fts_entries": health.fts_entries,
