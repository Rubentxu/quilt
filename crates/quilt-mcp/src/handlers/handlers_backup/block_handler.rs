...
impl DefaultBlockHandler {
    ...
    pub fn new(block_repo: Arc<dyn BlockRepository>, timezone_service: Arc<dyn Clock>) -> Self {
        Self {
            block_repo,
            timezone_service,
        }
...
impl BlockHandler for DefaultBlockHandler {
    ...
    async fn link_blocks(&self, source_id: Uuid, target_id: Uuid) -> HandlerResult {
        ...
        if !source.taxonomy.refs.contains(&target_id) {
            source.taxonomy.refs.push(target_id);
            self.block_repo
                .update(&source)
                .await
                .map_err(|e| e.to_string())?;
        ...
        let items: Vec<serde_json::Value> = blocks
            ...
            .map(|b| {
                serde_json::json!({
                    ...
                    "page_id": b.hierarchy.page_id.to_string(),
                    "content": b.content.as_ref(),
                    "deleted_at": b.timestamps.updated_at.to_rfc3339(),
                })
            })
            .collect();
        ...
        let items: Vec<serde_json::Value> = blocks
            ...
            .map(|b| {
                serde_json::json!({
                    ...
                    "page_id": b.hierarchy.page_id.to_string(),
                    "content": b.content.as_ref(),
                    "created_at": b.timestamps.created_at.to_rfc3339(),
                })
            })
            .collect();
