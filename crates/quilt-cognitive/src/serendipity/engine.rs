//! SerendipityEngine — Core Engine

use crate::ai_client::AIClient;
use crate::serendipity::types::{
...
/// Compare two Uuids by their inner bytes
...
pub fn temporal_proximity(a: &Block, b: &Block, halflife_days: f64) -> f32 {
    ...
    // Standard halflife decay: 0.5^(diff/halflife)
...
impl std::fmt::Debug for SerendipityEngine {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SerendipityEngine")
...
impl SerendipityEngine {
    ...
    pub async fn find_connections(
        ...
        let blocks = match query.temporal_window_days {
            ...
            None => {
                // page_id is required when no temporal window is specified
                let page_id = query.page_id.ok_or_else(|| {
                    SerendipityError::InvalidQuery(
        ...
        for (i, block_a) in blocks.iter().enumerate() {
            for block_b in blocks.iter().skip(i + 1) {
                ...
                let bridge = if composite >= self.options.min_bridge_threshold {
                    self.ai_client
                        .bridge_concept(block_a, block_b)
                        .await
                        .ok()
                        .flatten()
                } else {
                ...
                let bridge_concept = bridge.clone();
                let connection_type = if bridge_concept.is_some() {
                ...
                connections.push(SerendipityConnection {
                    ...
                    explanation: format_serendipity_explanation(structural, temporal, &bridge),
                    connection_type,
                });
            }
        }
...
fn format_serendipity_explanation(
    structural: f32,
    temporal: f32,
...
mod tests {
    ...
    use crate::serendipity::types::SerendipityQuery;
    ...
    fn test_temporal_halflife_apart() {
        ...
        let a = Block {
            ...
            marker: None,
            priority: None,
            content: quilt_domain::content::BlockContent::from_text("A"),
            properties: HashMap::new(),
            refs: vec![],
            tags: vec![],
            scheduled: None,
        ...
        let b = Block {
            ...
            marker: None,
            priority: None,
            content: quilt_domain::content::BlockContent::from_text("B"),
            properties: HashMap::new(),
            refs: vec![],
            tags: vec![],
            scheduled: None,
    ...
    fn test_composite_score() {
        let score = composite_score(0.5, 0.5);
        assert_eq!(score, 0.5);
    }

    #[test]
    fn test_composite_score_structural_heavy() {
        let score = composite_score(1.0, 0.0);
        assert_eq!(score, 0.6);
    }
    ...
    async fn test_find_connections_excludes_direct_refs() {
        ...
        let engine = SerendipityEngine::new(repo, ai);
    ...
    #[tokio::test]
    async fn test_find_connections_bridge_above_threshold() {
        let page_id = uuid_from_u8(1);
        let refs = vec![uuid_from_u8(100), uuid_from_u8(101), uuid_from_u8(102)];
        let a = make_block(uuid_from_u8(10), refs.clone(), page_id);
        let b = make_block(uuid_from_u8(11), refs, page_id);
        let blocks = vec![a, b];
        let repo = Arc::new(MockBlockRepo { blocks });
        ...
        let results = engine.find_connections(query).await.unwrap();
        assert!(!results.is_empty());
        assert!(results[0].bridge_concept.is_some());
    }
}
