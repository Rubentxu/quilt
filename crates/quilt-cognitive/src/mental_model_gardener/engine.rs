...
use crate::mental_model_gardener::types::*;
...
/// The Mental Model Gardener tracks belief evolution over time from journal pages.
#[derive(Clone)]
#[allow(dead_code)]
pub struct MentalModelGardener {
    block_repo: Arc<dyn BlockRepository>,
    agent_memory: Arc<AgentMemory>,
    ai_client: Arc<dyn AIClient>,
}

impl std::fmt::Debug for MentalModelGardener {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("MentalModelGardener")
            .field("block_repo", &"Arc<dyn BlockRepository>")
            .field("agent_memory", &"Arc<AgentMemory>")
            .field("ai_client", &"Arc<dyn AIClient>")
            .finish()
    }
}

impl MentalModelGardener {
    /// Create a new MentalModelGardener.
    pub fn new(
        block_repo: Arc<dyn BlockRepository>,
        agent_memory: Arc<AgentMemory>,
        ai_client: Arc<dyn AIClient>,
    ...
    pub async fn extract_beliefs(
        &self,
        agent_id: &str,
    ) -> Result<Vec<ExtractedBelief>, MentalModelError> {
        ...
        let blocks = self
            ...
            .await
            .map_err(MentalModelError::Repository)?;
        ...
        let beliefs = seen_concepts.into_values().collect();
        Ok(beliefs)
    }
    ...
    /// AI-driven belief extraction from text.
    async fn ai_extract_belief(&self, content: &str) -> (String, f64) {
        let prompt = format!(
            "Extract the core belief or opinion from this text. \
             Return ONLY a JSON object with 'belief' (the extracted belief statement, max 100 chars) \
             and 'confidence' (0.0-1.0) fields. \
             If no clear belief is expressed, return {{\"belief\": \"\", \"confidence\": 0.0}}. \
             Text: {}",
            content.chars().take(500).collect::<String>()
        );

        let system_prompt =
            "You are a belief extraction expert. Identify the core belief or opinion in text.";
        ...
        match self.ai_client.chat(system_prompt, &prompt).await {
            Ok(response) => {
                if let Ok(json) = serde_json::from_str::<serde_json::Value>(&response) {
                    ...
                    if !belief.is_empty() && confidence > 0.0 {
                        return (belief, confidence);
                    }
                }
                // Fall back to heuristic if AI didn't return useful result
                self.heuristic_belief_extract(content).await
            }
            Err(_) => {
    ...
    /// Heuristic-based belief extraction fallback.
    async fn heuristic_belief_extract(&self, content: &str) -> (String, f64) {
    ...
    /// Build a complete mental model for an agent.
    #[instrument(skip(self))]
    pub async fn build_model(&self, agent_id: &str) -> Result<MentalModel, MentalModelError> {
    ...
    ) -> Result<Vec<BeliefSnapshot>, MentalModelError> {
        let beliefs = self.extract_beliefs(agent_id).await?;
    ...
    /// Detect contradictions in an agent's mental model.
    #[instrument(skip(self))]
    pub async fn detect_contradictions(
        &self,
        agent_id: &str,
    ) -> Result<Vec<Contradiction>, MentalModelError> {
        let beliefs = self.extract_beliefs(agent_id).await?;
        let now = chrono::Utc::now();
    ...
    /// Suggest areas where the mental model is shallow.
    #[instrument(skip(self))]
    pub async fn suggest_deepening(
        &self,
        agent_id: &str,
        depth_threshold: usize,
    ) -> Result<Vec<DeepeningSuggestion>, MentalModelError> {
        let beliefs = self.extract_beliefs(agent_id).await?;
...
/// Errors for the Mental Model Gardener.
...
mod tests {
    ...
    fn make_test_gardener(blocks: Vec<Block>) -> MentalModelGardener {
        let repo = Arc::new(MockBlockRepo { blocks });
        let ai = Arc::new(crate::ai_client::MockAIClient::new());
        let agent_memory = Arc::new(AgentMemory::new(repo.clone(), ai.clone()));
        MentalModelGardener::new(repo, agent_memory, ai)
    }
