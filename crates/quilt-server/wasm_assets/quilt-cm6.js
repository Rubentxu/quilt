var __quiltCm6Bundle = (() => {
  ...
  var setDecorationsEffect = StateEffect.define();
  var decorationField = StateField.define({
    create() {
  ...
  function toggleFormatting(view, openMarker, closeMarker) {
    ...
    if (innerLen > 0 && selectedText.startsWith(openMarker) && selectedText.endsWith(closeMarker)) {
  ...
  function buildKeymap2(cbs) {
    const bindings = [];
    if (cbs.onUndo) {
      bindings.push({ key: "Mod-z", run: () => {
        cbs.onUndo();
        return true;
      } });
    }
    if (cbs.onRedo) {
      bindings.push({ key: "Mod-Shift-z", run: () => {
        cbs.onRedo();
        return true;
      } });
      bindings.push({ key: "Mod-y", run: () => {
        cbs.onRedo();
        return true;
      } });
    }
    ...
    if (cbs.onAcSelect) {
      bindings.push({
        ...
        run: () => {
          const entry = editors.get(currentEditingId);
          if (entry?.dropdownActive) {
            console.log("[CM6-AC-ENTER] Dropdown active, consuming Enter");
            cbs.onAcSelect();
            return true;
          }
          console.log("[CM6-AC-ENTER] Dropdown not active, falling through");
          return false;
        }
      });
    }
    ...
    if (cbs.onEnter) {
      bindings.push({
        key: "Enter",
        run: () => {
          const offset = currentEditingId !== null ? editors.get(currentEditingId)?.view.state.selection.main.anchor ?? 0 : 0;
          cbs.onEnter(offset);
          return true;
        }
      });
    ...
    if (cbs.onTab) {
      bindings.push({ key: "Tab", shift: false, run: () => {
        cbs.onTab();
        return true;
      } });
    }
    if (cbs.onShiftTab) {
      bindings.push({ key: "Tab", shift: true, run: () => {
        cbs.onShiftTab();
        return true;
      } });
    }
    if (cbs.onEscape) {
      bindings.push({ key: "Escape", run: () => {
        cbs.onEscape();
        return true;
      } });
    }
    if (cbs.onCtrlBackspace) {
      bindings.push({ key: "Mod-Backspace", run: () => {
        cbs.onCtrlBackspace();
        return true;
      } });
    }
  ...
  var api = {
     ...
     */
    createEditor(container, content2, callbacks) {
      const id = nextId++;
      const cbs = callbacks || {};
      const state = EditorState.create({
        ...
        extensions: [
          ...
          EditorView.domEventHandlers({
            keydown: (event, view2) => {
              if (event.key === "Tab") {
                if (event.shiftKey && cbs.onShiftTab) {
                  cbs.onShiftTab();
                  return true;
                } else if (!event.shiftKey && cbs.onTab) {
                  cbs.onTab();
                  return true;
                }
                return true;
              ...
              if (event.key === "Enter") {
                event.preventDefault();
                if (event.shiftKey) {
                  const entry2 = editors.get(id);
                  if (entry2?.view) {
                    const pos = entry2.view.state.selection.main.head;
                    entry2.view.dispatch({
    ...
    setContent(id, content2) {
      ...
      entry.cachedContent = content2;
    },
    /**
     * Set content and place the cursor at a specific offset.
     * Does NOT fire the onChange callback.
     */
    setContentAndCursor(id, content2, cursorOffset) {
      const entry = editors.get(id);
      if (!entry) return;
      const { view } = entry;
      view.dispatch({
